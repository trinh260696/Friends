...
{
    public class VKQRCodeDecodeController : MonoBehaviour
    {
        public VKDeviceCamera deviceCamera = null;
        ...
        if (framerate++ % 20== 0) {
#endif
                if (deviceCamera.isPlaying && !decoding)
                {
                    W = deviceCamera.dWebCam.Width();      // get the image width
                    H = deviceCamera.dWebCam.Height();     // get the image height 
                    ...
                    orginalc = deviceCamera.dWebCam.GetPixels(posx, posy, blockWidth, blockWidth);// get the webcam image colors
