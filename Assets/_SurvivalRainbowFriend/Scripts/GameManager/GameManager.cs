...
public class GameManager : MonoBehaviour
{
    ...
    private void GeneratePlayer()
    {
        ...
        var animator = Instantiate(Resources.Load<GameObject>("Friends/" + UserData.Instance.GameData.currentSkin.SkinObject.name), go.transform, false);
    ...
    #region Hamilton
    private void GenerateHamiltonBossPath()
    {
        //int n = BossWaypoints.Length + 1;
        ...
        //    for (int j = i + 1; j < n; j++)
        //    {
        //        float distance = Vector2.Distance((Vector2)BossWaypoints[i - 1], BossWaypoints[j - 1]);
        //        var raycastHit = Physics2D.Raycast(BossWaypoints[i - 1], BossWaypoints[j - 1] - BossWaypoints[i - 1], distance, LayerMask.GetMask("Wall"));
        //        if (raycastHit.collider == null)
        //        {
        //            tmp[i, j] = true; tmp[j, i] = true;
        //        }
        //    }
        //}
        //HamiltonBossCycle = HAMILTON.GetListHamilton(tmp, n);
          

    }
    private void GenerateHamiltonAllyPath()
    {
        //int n = AllyWaypoints.Length + 1;
        //bool[,] tmp = new bool[n, n];
        //for (int i = 0; i < n; i++)
        //{
        //    for (int j = 0; j < n; j++)
        ...
        //    for (int j = i + 1; j < n; j++)
        //    {
        //        float distance = Vector2.Distance((Vector2)AllyWaypoints[i - 1], AllyWaypoints[j - 1]);
        //        var raycastHit = Physics2D.Raycast(AllyWaypoints[i - 1], AllyWaypoints[j - 1] - AllyWaypoints[i - 1], distance, LayerMask.GetMask("Wall"));
        //        if (raycastHit.collider == null)
        //        {
        //            tmp[i, j] = true; tmp[j, i] = true;
        //        }
        //    }
        //}
        //HamiltonAllyCycle = HAMILTON.GetListHamilton(tmp, n);
           
                  
    }
    #endregion
    int count = 0;
    public void AddMission(TypeMission typeMission, int id=-1)
    {
        if (!StaticData.IsPlay) return;
        if (id != -1)
        {
        ...
        for (int i=0; i<StatusMissions.Count; i++)
        {
            if (StatusMissions[i].typeMission == typeMission)
            {
                ...
                uiPlay.UpdateMission(typeMission, StatusMissions[i].missionNumber, levelData.Items[i].missionNumber);
                break;
            }
        }     
        if (CheckWin())
